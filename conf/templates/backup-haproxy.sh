...
HOST_NAME="{{ HOST_NAME }}"
CLOUDFLARE_ACCOUNT_ID="{{ HAPROXY_BACKUP_CLOUDFLARE_ACCOUNT_ID }}"
BUCKET_NAME="{{ HAPROXY_BACKUP_BUCKET_NAME }}"

export AWS_ACCESS_KEY_ID="{{ HAPROXY_BACKUP_AWS_ACCESS_KEY_ID }}"
export AWS_SECRET_ACCESS_KEY="{{ HAPROXY_BACKUP_AWS_SECRET_ACCESS_KEY }}"
export AWS_DEFAULT_REGION="{{ HAPROXY_BACKUP_AWS_DEFAULT_REGION | default('auto') }}"

TELEGRAM_BOT_TOKEN="{{ HAPROXY_BACKUP_TELEGRAM_BOT_TOKEN | default('') }}"
TELEGRAM_CHAT_ID="{{ HAPROXY_BACKUP_TELEGRAM_CHAT_ID | default('') }}"
...
if [ -z "$S3_ENDPOINT" ]; then echo S3_ENDPOINT not set, exiting.; exit 1; fi
if [ -z "$BUCKET_NAME" ]; then echo BUCKET_NAME not set, exiting.; exit 1; fi
