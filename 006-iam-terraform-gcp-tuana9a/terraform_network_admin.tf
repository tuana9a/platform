resource "google_service_account" "terraform_network_admin" {
  account_id = "terraform-network-admin"
}

resource "google_project_iam_member" "terraform_network_admin_1" {
  project = "tuana9a"
...
resource "google_project_iam_member" "terraform_network_admin_2" {
  project = "tuana9a"
  # https://cloud.google.com/storage/docs/access-control/iam-roles
...
resource "google_service_account_iam_binding" "terraform_network_admin_claims" {
  service_account_id = google_service_account.terraform_network_admin.name
